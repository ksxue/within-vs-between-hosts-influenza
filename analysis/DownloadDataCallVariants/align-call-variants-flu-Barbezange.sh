...
# for the Barbezange et al. 2018., Front in Microbiol dataset.


# Barbezange et al. 2018, Frontiers in Microbiology
# Note that reads for this dataset are in single-end format.
# According to the published paper, the uploaded reads
# have already been cleaned of PCR primer sequences
# as well as non-influenza contaminants.
...
BatchInferSubtype="scripts/InferSubtype/Batch-InferSubtype-SingleEnd.sh"
BatchAlignSummarizeRealign="pipelines/Batch-AlignSummarizeRealign-SingleEnd.sh"
...
:<<END
# Omit this step because the reads are pre-trimmed in the database download.
...
if [ ! "$(ls ${dir}/*-realigned-annotated.summary | wc -l)" -eq ${numsamples} ] && \
  [ "$(ls nobackup/${project}/raw/*.fastq.gz | wc -l)" -eq $(( numsamples )) ] && \
  [ -f data/metadata/${project}/samplesheet-refs.txt ]; then
  ...
  qsub -cwd -N AlignReads -l h_rt=96:00:00 \
    -t 1-${numsamples} -tc 300 -l m_mem_free=12G \
    -o nobackup/sge -e nobackup/sge \
