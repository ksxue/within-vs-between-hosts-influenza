...
colnames(Data) <- c("Chr","Pos","Base","RefBase","GenomePos","Count","AvgQ","AvgReadPos",
                    ...
                    "Sample")
# For longitudinal samples, set the consensus equal to the reference base.
# This assumes that the reads have been mapped to the reference genome
# at the initial timepoint.
Data <- Data %>% filter(Gene != "none") %>% group_by(Gene, Pos) %>%
  mutate(Consensus=RefBase, Freq=Count/sum(Count),
         Coverage=sum(Count))
