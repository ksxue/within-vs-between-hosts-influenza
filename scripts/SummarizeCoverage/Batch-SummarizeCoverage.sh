...
# Script for batch submission of SummarizeCoverage jobs.

# Input parameters.
pipeline="scripts/SummarizeCoverage/SummarizeCoverage.r"
...
mincoverage=$4 # Give the minimum coverage at a site required to call a variant.
...
Rscript ${pipeline} ${indir}/${sample}-${refname}-realigned-annotated.summary \
  ${outdir}/${sample}-${refname}.coverage ${mincoverage}