...
# Input parameters.
pipeline="pipelines/AlignSummarizeRealign-Longitudinal.sh"
...
initialtimepoint=`cut -f9 -d' ' ${samplesheet} | awk "NR==$SGE_TASK_ID"` # Extract initial timepoint sample name.
${pipeline} ${fastq1} ${fastq2} ${sample} ${dir} \
  ${reference} ${refname} ${reference}.bed ${initialtimepoint} ${clean}